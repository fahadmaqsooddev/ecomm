        ...
        <div class="col-12">
          <div class="card mt-5 p-2">
            <div class="card-header">
              <h3 class="card-title mt-2">Brands</h3>
              <div class="row">
              ...
              <div class="table-responsive">
                <table id="example2" class="table table-bordered table-hover w-100">
                  ...
                  <tbody>
                    @if($brands)
                      @foreach($brands as $key => $brand)
                        <tr>
                          <td>{{ $brands->currentPage() * $brands->perPage() - $brands->perPage() + $key + 1 }}</td>
                          <td><img src="{{ asset('admin/dist/img/'.$brand->image) }}" width="80" height="80"></td>
                          <td>{{ $brand->name }}</td>
                          <td><a href="{{ route('admin.editbrand',$brand->id)}}" class="btn btn-sm btn-primary">Edit</a></td>
                          <td><a href="{{ route('admin.deletebrand',$brand->id)}}" class="btn btn-sm btn-danger">Delete</a></td>
                        </tr>    
                      @endforeach
                    @endif
                  </tbody>
                 ...
                 <div class="d-flex justify-content-end">
                    {{ $brands->links('pagination::bootstrap-4') }} <!-- Use Bootstrap 4 pagination -->
                </div>
